...
/// Parses Story Navigator / Story Reader library filenames of the form:
///   "{Title} #tag1 #tag2 … (id).txt"
/// and derives a series key ("base title") so related parts group as one story.
///
/// Ported verbatim from the Story Reader app's `FilenameParser` (verified
/// against 25,489 files) so grouping matches the source app exactly.
struct ParsedStoryName {
    var title: String
    var tags: [String]
    var storyID: String?
}

enum StoryFilenameParser {
    ...
    static func parse(stem: String) -> ParsedStoryName {
        var s = stem
        ...
        guard let m = fnameRe.firstMatch(in: s, range: NSRange(location: 0, length: ns.length)) else {
            return ParsedStoryName(title: s.trimmingCharacters(in: .whitespaces), tags: [], storyID: nil)
        }
        ...
        return ParsedStoryName(title: title, tags: tags, storyID: id)
    }
